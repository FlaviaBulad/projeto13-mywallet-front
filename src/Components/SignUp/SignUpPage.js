...
export default function SignUpPage() {  //Main function,reders the sigh up page
    ...
    function SignUpDataToAPI(e) { //sending all the data to api and redirectioning user to login page if successfull
        e.preventDefault();
        setIsLoading(true);
        const promise = axios.post("https://localhost:5000/sign-up", { ...signUpData });
        promise.then(() => {
            setIsLoading(false);
            navigate("/");
        });
        ...
        return (
            <>
                ...
                <Input
                    ...
                    />
                       <Input
                        type="password" placeholder="Confirme a senha" name="password_confirmation"
                        onChange={OnChange} value={signUpData.password_confirmation} required
                    />
